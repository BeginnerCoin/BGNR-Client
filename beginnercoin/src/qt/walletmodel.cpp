...
{
    CBeginnercoinAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered beginnercoin address / amount:
            ...
            scriptPubKey.SetDestination(CBeginnercoinAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal beginnercoin:URI (beginnercoin:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CBeginnercoinAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CBeginnercoinAddress(address).ToString().c_str()].push_back(out);
    }
}
...
{
    CTxDestination dest = CBeginnercoinAddress(sAddress).Get();
