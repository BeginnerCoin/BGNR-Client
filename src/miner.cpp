...
//    txNew.vout[0].scriptPubKey = scriptPubKeyIn;
    vector<string> coinAddressStrings;
    txNew.vout.resize(1);
    txNew.vout[0].scriptPubKey = scriptPubKeyIn;
//	
//	// Prepare to pay beneficiaries
    ...
    for (unsigned int i=0; i<coinAddressStrings.size(); i++)
    {
        ...
        // Create transaction
        CKeyID addrKeyId;
		CBeginnercoinAddress addr(coinAddressString);
		if(!addr.IsValid())
		{
			LogPrintf("CBeginnercoinAddress not valid! %s\n", coinAddressString.c_str());
			return NULL;
		}
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BeginnercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BeginnercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BeginnercoinMiner(CWallet *pwallet)
{
    LogPrintf("BeginnercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("beginnercoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running BeginnercoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BeginnercoinMiner terminated\n");
        throw;
    }
}

void GenerateBeginnercoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BeginnercoinMiner, pwallet));
}
