...
BEGINNERCOIND=${1}/beginnercoind
CLI=${1}/beginnercoin-cli
...
B4ARGS="-datadir=$D4"
$BEGINNERCOIND $BEGINNERCOINDARGS $B4ARGS &
B4PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} bgnr"
...
function StartThree {
  $BEGINNERCOIND $BEGINNERCOINDARGS $B1ARGS &
  B1PID=$!
  $BEGINNERCOIND $BEGINNERCOINDARGS $B2ARGS &
  B2PID=$!
  $BEGINNERCOIND $BEGINNERCOINDARGS $B3ARGS &
  B3PID=$!
}
